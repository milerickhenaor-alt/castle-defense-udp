"""
view/renderer/PlayerView.py

Principios SOLID aplicados
--------------------------
SRP : PlayerView solo se encarga de cargar y dibujar el sprite del jugador.
      No conoce la lógica del juego ni la red.
LSP : Implementa IDrawable. El Renderer puede usarlo como IDrawable
      sin saber que es específicamente un jugador.
...
class PlayerView(IDrawable):
    """
    Vista del jugador — carga animaciones y dibuja el sprite en pantalla.
    ...
    def __init__(self, player_data):
        ...
        # 3. MAPEAR CARPETAS CORRECTAS SEGÚN ESTRUCTURA DE ASSETS
        if tipo == "Fairy":
            folder = "Fairies"
        elif tipo == "Gent":
            folder = "Gentlemen"
    ...
    def update(self, *args, **kwargs) -> None:
        """
        [IDrawable] Actualiza el frame de la animación.
        Prioriza caminar, si no hay frames de caminar usa idle.
        SRP: solo actualiza la animación, no mueve al jugador.
        """
    ...
    def draw(self, screen: pygame.Surface, x=0, y=0, team="A", *args, **kwargs) -> None:
        """
        [IDrawable] Dibuja el sprite en pantalla.
        - team: 'A' o 'B' para voltear el sprite.
        - x, y: coordenadas que vienen del modelo.
        SRP: solo dibuja, no modifica el estado del jugador.
        """
        if not self.image:
            return
